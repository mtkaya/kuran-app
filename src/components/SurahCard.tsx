...
export const SurahCard: React.FC<SurahCardProps> = ({ surah, variant = 'list' }) => {
    ...
    if (variant === 'grid') {
        return (
            ...
            >
                {/* Arabic Name - Forced 6px */}
                <span
                    className="font-arabic !text-[6px] text-foreground text-center leading-none mt-1 line-clamp-1 w-full"
                    dir="rtl"
                    style={{ fontSize: '6px' }}
                >
                    {surah.name_arabic}
                ...
                {/* Surah Name - Turkish - 6px */}
                <h3 className="font-medium !text-[6px] text-muted-foreground text-center leading-tight line-clamp-2 w-full px-0.5" style={{ fontSize: '6px' }}>
                    {surah.name_turkish}
                </h3>
            </Link>
        );
