...
export const SurahCard: React.FC<SurahCardProps> = ({ surah, variant = 'list' }) => {
    ...
    if (variant === 'grid') {
        return (
            ...
            >
                {/* Arabic Name - 6px as requested */}
                <span className="font-arabic text-[6px] text-foreground text-center leading-none mt-1 line-clamp-1 w-full" dir="rtl">
                    {surah.name_arabic}
                </span>

                {/* Surah Name - Turkish - 6px as requested */}
                <h3 className="font-medium text-[6px] text-muted-foreground text-center leading-tight line-clamp-2 w-full px-0.5">
                    {surah.name_turkish}
                </h3>
            </Link>
        );
